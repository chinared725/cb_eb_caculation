...
data.drop(['adjust_tip', 'adjusted', 'apply_cd', 'fund_rt', 'last_time', 'left_put_year', 'margin_flg', 'noted', 'online_offline_ratio', 'option_tip', 'owned',\
           'pre_bond_id', 'put_notes', 'qflag', 'qflag2', 'qstatus', 'real_force_redeem_price', 'ref_yield_info', 'repo_cd', 'sqflg', 'stock_cd', 'stock_net_value', \
             'price','put_dt','put_inc_cpn_fl','put_total_days', 'rating_cd','ration', 'redeem_dt', 'redeem_inc_cpn_fl',\
           'repo_valid',  'repo_valid_from', 'repo_valid_to','ration_cd', 'turnover_rt'], axis=1, inplace=True)
...
cols = {'adj_cnt':'下调次数', 'adj_scnt':'下调成功次数', 'convert_cd':'转债占比',  'convert_cd_tip':'转股提示', 'convert_dt':'转股期开始', 'convert_price':'转股价', 'convert_price_valid':'转股价有效性',\
 ...
 'volume':'可转债成交_万', 'year_left':'剩余年限', 'convert_amt_ratio':'可转债_股票市值比', 'premium_rt':'溢价率', 'sincrease_rt':'正股涨幅', 'ytm_rt':'年化税前收益率', 'ytm_rt_tax':'年化税后收益率','bond_id':'转债代码','bond_nm':'转债名称',\
        'increase_rt':'转债涨幅','price_tips':'上市状态'}
...
cb_price_ascending = cb[['转债代码', '转债名称', '转债价格','正股现价','转股价','转股价值','溢价率','转债涨幅','正股涨幅','评级','剩余年限','当前规模_亿','原始规模_亿','双低',\
                   '正股名字','正股现价','pb','换手率','上市状态','下调次数','下调成功次数']].sort_values(by=['转债价格', '溢价率'])

cb_double_low= cb_price_ascending[(cb_price_ascending['转债价格']<106)  & (cb_price_ascending['溢价率']<0.3) \
                                            & (cb_price_ascending['上市状态']!='待上市')]

cb_adjusted = cb_price_ascending[(cb_price_ascending['下调次数']>=1) & (cb_price_ascending['转债价格'] < 110) & (cb_price_ascending['溢价率']<0.35)].sort_values(by=['转债价格', '溢价率'])
